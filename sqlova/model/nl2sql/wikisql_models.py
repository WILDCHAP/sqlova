...
class SAP(nn.Module):
    ...
    def forward(self,
                wemb_n,     #从bert中输出的问题的参数
                l_n,        #问题的长度
                wemb_hpu,   #从bert中输出的表字段的参数
                l_hpu,      #表头字段的长度
                l_hs,       #表字段总数
                pr_sc,      #每个问题所问的字段
                show_p_sa=False):
        # LSTM层======问题输出->[16, 43, 100]
        # 16为batch_size
        # 43?
        # 100为在进行column_attention的时候，隐藏层的个数
        wenc_n = encode(self.enc_n, wemb_n, l_n,
                        return_hidden=False,
                        hc0=None,
                        last_only=False)  # [b, n, dim]
        # LSTM层======表头输出->[16, 18, 100]
        # 16为batch_size
        # 18?
        # 100为在进行column_attention的时候，隐藏层的个数
        wenc_hs = encode_hpu(self.enc_h, wemb_hpu, l_hpu, l_hs)  # [b, hs, dim]
        ...
        mL_n = max(l_n) #最大的问题的长度
        #这里应该是二维[16, 100]而不是[16, 16, 100]
        wenc_hs_ob = wenc_hs[list(range(bS)), pr_sc]  # 从上面得到的表头输出中得出：每一个问题他问的某个表头的参数[16, 16, 100]

        # [bS, mL_n, 100] * [bS, 100, 1] -> [bS, mL_n] || [16, 43, 100] * [16, 100, 1] -> [16, 43]
        att = torch.bmm(self.W_att(wenc_n), wenc_hs_ob.unsqueeze(2)).squeeze(2)

        # 对bat_size个问题的长度加入att？
        for b, l_n1 in enumerate(l_n):
            if l_n1 < mL_n:
