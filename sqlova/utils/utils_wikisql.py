...
from annotate_ch_corenlp import get_star_and_end
...
# 加载数据本体（加载train、dev等各种数据，取决于mode）
def load_wikisql_data(path_wikisql, mode='train', toy_model=False, toy_size=10, no_hs_tok=False, aug=False):
    ...
    #path_sql = os.path.join(path_wikisql, mode+'_tok.jsonl')            #已经分词了的问题
    '''这里暂时使用没分词的'''
    path_sql = os.path.join(path_wikisql, mode+'.json')

    if no_hs_tok:
        path_table = os.path.join(path_wikisql, mode + '.tables.json') #表格是否分词？
    else:
    ...
    data = []   #mode_data
    table = {}  #mode_table
    with open(path_sql, 'rb') as f:   #先处理问题文件
        for idx, line in enumerate(f):
            if toy_model and idx >= toy_size:   #??
                break

            t1 = json.loads(line.strip())   #利用json.loads将json分成Key-Value格式
                                            #load函数和loads函数区别是，load里面直接写json文件，而loads里面写String
            '''这里向t1中添加一列：ch_corenlp(list)'''
            '''2020/11/28修改：如果where-value值未找到，则舍弃掉该条记录'''
            '''                 如果select-col==2，也舍弃'''
            '''                 如果where之间条件链接是2(or)也舍弃'''
            # list_b_2_e = get_star_and_end(t1)
            # t_temple = {'ch_corenlp': list_b_2_e}
            if(len(t1['sql']['sel']) != 1):
                continue
            if(t1['sql']['cond_conn_op'] == 2):
                continue
            list_b_2_e = get_star_and_end(t1)
            if(list_b_2_e == -1):
                continue
    ...
    with open(path_table, 'rb') as f:
        for idx, line in enumerate(f):
            ...
            table[t1['id']] = t1            #这里注意直接用表的id作为list的下标，即 model_table[表ID] = 表
...
#将问题的data文件转换成‘更专业’的loader文件，shuffle表示是否打乱
def get_loader_wikisql(data_train, data_dev, bS, shuffle_train=True, shuffle_dev=False):
    train_loader = torch.utils.data.DataLoader(
        batch_size=bS,          #每次输入数据的行数，默认为1。PyTorch训练模型时调用数据不是一行一行进行的（这样太没效率），而是一捆一捆来的。这里就是定义每次喂给神经网络多少行数据
        dataset=data_train,     #就是没经过pytorch处理的数据
    ...
    数据加载器，结合了数据集和取样器，并且可以提供===!!!多个线程处理数据集!!!===
    它把训练数据分成多个小组(batch_size)，此函数每次抛出一组数据，后续只需要再包装成Variable即可作为模型的输入
    """
...
def get_fields_1(t1, tables, no_hs_t=False, no_sql_t=False):
    ...
    '''分成一个一个字(不分词)待改进'''
    nlu_t1 = t1['question']
    #nlu_t1 = nlu_t1.strip().splite('')
    nlu_t1 = list(nlu_t1)

    tid1 = t1['table_id']

    '''这里要注意中文agg中还分出了一个字段，要么在这里改，要么就在下面算每个bs问题sc数那里改'''
    sql_i1 = t1['sql']
    sql_q1 = t1['sql']    #sql和query一样
    if no_sql_t:
        sql_t1 = None
    else:
        sql_t1 = t1['query_tok']

    tb1 = tables[tid1]      #获取该问题对应的表
    if not no_hs_t:
...
def get_g(sql_i):
    ...
    for b, psql_i1 in enumerate(sql_i):
        g_sc.append( psql_i1["sel"][0] )    #先只选择一个（可以选两个，待改进）
...
def get_g_wvi_corenlp(t):
    g_wvi_corenlp = []
    for t1 in t:
        g_wvi_corenlp.append( t1['ch_corenlp'] )
...
def generate_sql_q1(sql_i1, tb1):
    ...
    if isinstance(sql_i1['agg'], int64):
        select_agg = agg_ops[sql_i1['agg']]
    else:
        select_agg = agg_ops[sql_i1['agg'][0]]
    if isinstance(sql_i1['sel'], int64):
